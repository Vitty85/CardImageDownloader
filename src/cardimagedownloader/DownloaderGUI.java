...
public class DownloaderGUI extends javax.swing.JFrame {
    ...
    public String getSpecialTokenUrl(String id){
        ...
        else if(id.equals("297164t") || id.equals("297181t")) //Phyrexian Germ 0/0
        ...
        else if(id.equals("296483t")) //Phyrexian Golem 3/3
    ...
    public Document findTokenPage(String imageurl, String name, String set, String tokenstats, String color) throws Exception {
        ...
        /*setTextArea("Warning: Token " + name + " has not been found between " + set + " tokens, i will search for it in https://deckmaster.info", Color.blue, new Font("Arial", 1, 14));
        String json = "";
        try {  
        ...
        for (int i = 0; i < urls.size() && !interrupted; i++){
            while(paused && !interrupted){
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {}
                ...
                if(stats != null){
                    for (int j = 0; j < stats.size() && !interrupted; j++){
                        ...
                        if(stats.get(j).attributes().get("content").contains("Token Creature") && 
                            ...
                            if(stats != null) {
                                for(int y = 0; y < stats.size() && !interrupted; y++){
                                    while(paused && !interrupted){
                                        try {
                                            Thread.sleep(1000);
                                        } catch (InterruptedException e) {}
                                    ...
                                    if(nodes != null){
                                        for(int p = 0; p < nodes.size() && !interrupted; p++){
                                            while(paused && !interrupted){
                                                try {
                                                    Thread.sleep(1000);
                                                } catch (InterruptedException e) {}
        ...
        setTextArea("Warning: Token " + name + " has not been found in https://deckmaster.info so i will search for it between any other set in " + imageurl + " (it may take a long time)", Color.blue, new Font("Arial", 1, 14));*/
                    ...
                    for (int y = 0; y < mappa.size() && !interrupted; y++) {
                        ...
                        /*if(false){
                            ...
                            try{
                                ...
                                setTextArea("Warning: Problem reading card (" + mappa.get(id) + ") infos from: " + deckutrl  + id + ", i will retry 2 times more...", Color.blue, new Font("Arial", 1, 14));
                                try{
                                    doc = Jsoup.connect(deckutrl + id).get();
                                } catch(Exception e2) {
                                    setTextArea("Warning: Problem reading card (" + mappa.get(id) + ") infos from: " + deckutrl  + id + ", i will retry 1 time more...", Color.blue, new Font("Arial", 1, 14));
                                    try{
                                        doc = Jsoup.connect(deckutrl + id).get();
                                    } catch(Exception e3) {
                                        setTextArea("Error: Problem reading card (" + mappa.get(id) + ") infos from: " + deckutrl  + id + ", i will not retry anymore...", Color.red, new Font("Arial", 1, 14));
                                        continue;
                                    }
                                }
                            }
